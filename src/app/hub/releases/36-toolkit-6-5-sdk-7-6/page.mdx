...
redeemed**, and only replaces it with the real on-chain amount when the bettor claims. Measured across
production bets, that figure is 17% to 33% too high.
...
- `getBetsReport` / `calcBetsReport` rebuild `returns` as `stake × calcMinOdds(surviving leg odds)` —
  ...
  product rather than compounded per leg. Against redeemed production combos with a voided leg, that
  lands within ~1% of the on-chain payout, against up to ~10% for a plain product of the odds.
