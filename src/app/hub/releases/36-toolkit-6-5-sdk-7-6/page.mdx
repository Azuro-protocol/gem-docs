...
ROI is settled-only (`profit / turnover × 100`, where settled means `Won`, `Lost`, `Cancelled`, or cashed
out) and is `null` — never `NaN` — when turnover is zero. Pending bets are excluded from ROI and
surfaced separately as `atStake`. Freebet-funded bets are excluded from every main figure too, since the
stake wasn't the bettor's own money, and are reported in their own `freebet` bucket instead.
