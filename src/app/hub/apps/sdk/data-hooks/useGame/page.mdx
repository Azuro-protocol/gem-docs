...
```ts
{
  gameId: string
  chainId?: ChainId
  query?: QueryParameterWithSelect<UseGameQueryFnData, TData> // useQuery params
}
```

```ts
type UseGameQueryFnData = GameData | null
```
...
`getUseGameQueryOptions` lets you build query options outside a component — useful for prefetching, SSR, or composing queries.

```ts
import { getUseGameQueryOptions } from '@azuro-org/sdk'

const options = getUseGameQueryOptions({ ...props, chainId })
await queryClient.prefetchQuery(options)
```

```ts
type GetUseGameQueryOptionsProps = UseGameProps & {
  chainId: ChainId
}
```
