...
type UseSportsProps = {
  ...
  chainId?: ChainId
  query?: QueryParameterWithSelect<UseSportsQueryFnData, TData> // useQuery params
}
```
...
const options = getUseSportsQueryOptions({ ...props, chainId })
await queryClient.prefetchQuery(options)
```

```ts
type GetUseSportsQueryOptionsProps = UseSportsProps & {
  chainId: ChainId
}
```
