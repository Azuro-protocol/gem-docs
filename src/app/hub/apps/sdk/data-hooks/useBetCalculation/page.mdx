...
```ts
{
  ...
  query?: QueryParameterWithSelect<UseBetCalculationQueryFnData, TData> // useQuery params
}
```

```ts
type UseBetCalculationQueryFnData = BetCalculation
```
...
`getUseBetCalculationQueryOptions` lets you build query options outside a component — useful for prefetching, SSR, or composing queries.

```ts
import { getUseBetCalculationQueryOptions } from '@azuro-org/sdk'

const options = getUseBetCalculationQueryOptions({ ...props, chainId })
await queryClient.prefetchQuery(options)
```

```ts
type GetUseBetCalculationQueryOptionsProps = UseBetCalculationProps & {
  chainId: ChainId
}
```
