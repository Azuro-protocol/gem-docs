...
<Callout type="info">
  **`payout` vs `settledPayout`**
  ...
  leg that has not been redeemed yet**. The indexer keeps the full pre-void payout for that shape until
  the bettor claims — 17% to 33% too high across production bets — so both figures are rebuilt from the
  surviving legs instead, at the same `totalOdds` this hook already reports. Redeemed bets are untouched.
</Callout>
