...
<Callout type="info">
  **`payout` vs `settledPayout`**
  ...
  leg that has not been redeemed yet**. The recorded payout still credits the voided leg until the bet is
  redeemed, so it overstates what the bettor can claim; both figures are rebuilt from the surviving legs
  instead, at the same `totalOdds` this hook already reports. Once redeemed, the recorded payout is the
  amount actually paid and is used as is.
</Callout>
