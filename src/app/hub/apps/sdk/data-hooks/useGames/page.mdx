...
type UseGamesProps = {
  ...
  chainId?: ChainId
  query?: QueryParameterWithSelect<UseGamesQueryFnData, TData> // useQuery params
}
```

```ts
type UseGamesQueryFnData = GetGamesByFiltersResult
```
...
`getUseGamesQueryOptions` lets you build query options outside a component — useful for prefetching, SSR, or composing queries.

```ts
import { getUseGamesQueryOptions } from '@azuro-org/sdk'

const options = getUseGamesQueryOptions({ ...props, chainId })
await queryClient.prefetchQuery(options)
```

```ts
type GetUseGamesQueryOptionsProps = UseGamesProps & {
  chainId: ChainId
}
```
