...
<Callout type="warning">
  ...
  Before utilizing `useLiveStatistics`, it is essential to initialize the `LiveStatisticsSocketProvider`.
  It opens and maintains the statistics socket itself — there is no separate watcher to mount. The example
  below also keeps the game's state live with `useGameState`, which needs `FeedSocketProvider` and
  `GameUpdatesProvider`:
...
const { gameId, state: initialState, sport: { sportId } } = game // game from useGame / useGames
...
```ts
{
  /** `undefined` until the first message arrives, `null` when the feed reports no stats for the game */
  data: LiveStatistics | null | undefined
  isFetching: boolean  // flag indicates statistics fetching
  isAvailable: boolean // flag indicates the availability of statistics
}
```
```ts
type LiveStatistics = {
...
// per-sport incident feed - see the `SoccerIncidens` / `BasketballIncidens` / `TennisIncidents` /
// `VolleyballIncidets` types, all exported from `@azuro-org/sdk`
type TimeLine = SoccerIncidens | BasketballIncidens | TennisIncidents | VolleyballIncidets
