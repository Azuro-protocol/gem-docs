...
Each Game has a `state` field that indicates their current state.
For instance, `GameState.Prematch` signifies the condition is currently in prematch state, while `GameState.Stopped` indicates it has temporarily stopped accepting bets.
