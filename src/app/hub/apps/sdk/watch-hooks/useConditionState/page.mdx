...
is [pointed at another condition](#pointing-the-hook-at-another-condition) — and then follows two rules.
...
<Callout type="info">
  A state read still in flight for the condition that has gone is discarded rather than written into the
  new one, so a slow read can't overwrite the condition you are now watching with the answer for a
  previous one.
</Callout>
