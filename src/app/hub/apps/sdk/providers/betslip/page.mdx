...
```tsx {1, 20, 22}
import { ChainProvider, FeedSocketProvider, GameUpdatesProvider, ConditionUpdatesProvider, BetslipProvider } from '@azuro-org/sdk'
...
enum BetslipDisableReason {
  ConditionState = 'ConditionState', // one or more selected conditions or outcomes are no longer bettable (stopped, hidden or already settled)
...
<Callout type="info">
`selectedFreebet` and `freebets` use the `Freebet` type returned by [useAvailableFreebets](/hub/apps/sdk/bonus/useAvailableFreebets) — see that hook for the full type shape, and the [freebets guide](/hub/apps/guides/freebets) for how freebets are selected and applied.
</Callout>
