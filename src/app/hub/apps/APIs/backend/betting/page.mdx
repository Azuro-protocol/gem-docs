...
const gameDetails = await fetch(`https://dev-api.onchainfeed.org/api/v1/public/market-manager/games-by-ids`, {
  method: 'POST',
  headers: {
...
const gameDataById = gameDetails.games.reduce((acc, game) => {
  acc[game.id] = game;
  return acc;
}, {})
...
const betsWithGameDetails = userBets.data.v3Bets.map(bet => {
  const games = bet._gamesIds.map(id => gameDataById[id]);
