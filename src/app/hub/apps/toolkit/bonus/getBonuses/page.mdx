...
Fetches all bonuses (currently limited to `freebets`) for a bettor account filtered by status.
By default, retrieves only available (not expired) bonuses. Returns `null` if no bonuses are found.
