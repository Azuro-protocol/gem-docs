...
[`calcMinOdds`](/hub/apps/toolkit/utils/calcMinOdds) for the minimum odds to place a new one.
...
calcComboOdds({ odds: [ 2, 1.75 ], createdAt: MARGIN_APPLIED_AT - 1 })  // plain product
```

Leave voided legs out of the array. An empty array prices at `1`, so a bet with nothing left standing
returns its stake. Full details on [the `calcComboOdds` page](/hub/apps/toolkit/utils/calcComboOdds).

### `MARGIN_APPLIED_AT`

Unix seconds, the moment the feed started applying its fee to every outcome's odds. Exported because
reconstructing any historical combo figure needs it.

### `BetsReportEntry.createdAt` is required

Only affects code that builds entries itself and calls `calcBetsReport` directly —
[`getBetsReport`](/hub/apps/toolkit/bet/getBetsReport) fills it in. Required rather than optional because
...
<Callout type="warning">
  **Use [`calcComboOdds`](/hub/apps/toolkit/utils/calcComboOdds), not `calcMinOdds`, to price a bet that
