...
A bet counts as settled once it is `Won`, `Lost`, `Cancelled`, or cashed out - checked in that order,
because a cashed-out bet keeps a notional settlement payout that would otherwise double-count it as
`Won` too. Pending bets never touch `turnover`, `returns`, or `roi` - they only contribute to
`pendingCount` and `atStake`, so ROI never has to guess at the outcome of a bet that hasn't resolved yet.

`profit = returns - turnover`, and `roi = profit / turnover * 100`, rounded to 2 decimals. Returns are
always read from the protocol's own recorded payout, never reconstructed from odds - a bet's payout is
not `amount * settledOdds`.
