...
The exception is the one shape where the recorded payout is not what the bet returns: **a winning combo
with at least one voided leg that has not been redeemed yet**. Until the bet is redeemed the recorded
payout still credits the voided leg's odds as if that leg had won, so it overstates returns. Those
