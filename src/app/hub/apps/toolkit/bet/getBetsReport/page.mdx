...
The exception is the one shape where the recorded payout is knowingly wrong: **a winning combo with at
least one voided leg that has not been redeemed yet**. Until redemption the indexer keeps the full
pre-void payout, crediting the voided leg's odds as if that leg had won - measured against production
that overstates returns by 17% to 33%. Those returns are rebuilt from the surviving legs with
`calcMinOdds`, which prices a combo the way the protocol does: the feed's 1% fee is removed per leg and
re-applied once to the product instead of compounding once per leg. Measured against redeemed
production combos with a voided leg, the rebuild lands within ~1% of the on-chain payout, against up to
~10% for a plain product of the surviving odds. Redeemed bets always read the recorded payout, which by
then is the amount actually paid out.
