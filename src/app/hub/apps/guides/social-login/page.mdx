...
To act with user's smart account, get a walletClient for the **required chain** via the
`getClientForChain` method of the `useAAWalletClients` hook from `@azuro-org/sdk-social-aa-connector`.
...
<Callout type="warning">
  **Always request the client by chain id — don't reuse a single ambient client.**

  The older `useAAWalletClient` (singular) hook returns one client bound to whatever chain Privy currently
  has selected, and Privy does **not** always switch to the chain your app needs. That mismatch makes
  transactions land on (or revert against) the wrong chain. `useAAWalletClients().getClientForChain({ id })`
  resolves a client explicitly for the chain you pass — pass your app chain id (`appChain.id`) so the
  transaction is always signed and sent on the intended chain. `useAAWalletClient` is **deprecated**; prefer
  `useAAWalletClients`.
</Callout>
...
import { useAccount, useAAWalletClients } from '@azuro-org/sdk-social-aa-connector'
...
export const WithdrawalForm = () => {
  ...
  const withdraw = async () => {
    // resolve the AA client for the chain we actually want to transact on
...
and work with user's smart account by viem walletClient drop-in replacement from
`useAAWalletClients().getClientForChain({ id })`.
...
const Component = () => {
  ...
  const sendTxExample = async () => {
    const to ='0x...'
    const data ='0x...'
    const chain = appChainTakenFromViem

    // resolve a client for the exact chain — Privy may not have switched to it
    const aaWalletClient = await getClientForChain({ id: chain.id })

    const txHash = isAAWallet
      ? await aaWalletClient.sendTransaction({ to, data, account: aaWalletClient.account, chain })
