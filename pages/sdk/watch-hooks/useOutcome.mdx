...
```
<Callout type="info">
  The `initialOdds` and `initialStatus` fields are both optional. If either is not provided, the `useOutcome` hook will automatically retrieve the initial values. The boolean flags `isOddsFetching` and `isStatusFetching` indicate whether the initial values are being fetched.
</Callout>
