...
To place an combo bet, you must call the `bet` function of the LP smart contract and provide the following parameters:
...
The main goal of the major release is to allow support for combo bets to existing within the same entity as single bets. In the initial version, all single bets were associated with a single `Outcome` entity. This needed to be clarified because the `Outcome` entity also had the `odds` field. GraphQL users might have mistakenly assumed that this field represented the bet odds.

To address this confusion, we are introducing the `Selection` entity. It combines the bet, bet odds, and outcome. Now, all bets contain a `selections: Selection[]` field that merges single and combo bets into a single entity. For a single bet, this field will contain only one selection entity item, while for an combo bet, it will contain two or more.
