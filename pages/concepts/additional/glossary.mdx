...
An automated market maker is a type of decentralized exchange that uses algorithms to determine the price of assets based on supply and demand. Azuro utilises AMM mechanic inside its predictions engine to adjust odds and provide users with correct values.

### Predictions

At its core, sports predictions are putting money behind an outcome of your choice and getting paid if that outcome is achieved.
...
Players can access the markets via websites, apps or widgets (known as "Frontends") that connect to Azuro to deliver
prediction services to end users.
...
compensated with the prediction margins.

### Margin

The margin (also referred to as "spread" or "vig/vigorish") is the difference between the probability of a prediction outcome happening and the odds that players can place predictions with (for this outcome). In traditional online betting, it is also referred to as the "bookmakers advantage" or "house edge".
On Azuro protocol, margin is one of the immutable properties of a Condition, set at the time of its creation.

### Market

A sports market is a specific category of gaming on a certain fixture or event.

### Odds

The ratio of winnings to the stake and player also get his wager returned. So wagering 1 at 1:5 pays out 6 (5 + 1).

### Order

The request to make a prediction, signed by user's wallet .

### Outcome (Selection)

The simplest type of prediction in an app’s office, the winning of which is determined by the result of a sporting
event - the victory of team(player) 1 or team 2(player), or a draw (if possible).

### Pool

A system of smart contracts that forms a unified predictions platform. Anyone can deploy their own Pool on the network
using the Factory.
