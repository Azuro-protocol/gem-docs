...
When a user deposits liquidity they are immediately getting exposed to losses that may occur on the markets that are already open at that time. The LP position starts getting exposure into pool winnings for markets created AFTER the position has been opened. This means that in the first few days it is likely that the liquidity position shows negative returns. The more time passes, the more this initial negative skew becomes negligible. I.e. the position becomes aligned with older liquidity positions’ profitability over time.
