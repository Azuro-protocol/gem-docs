...
First, the coefficients for each sub-bet are calculated without margin and with a zero value for "amount". 
Then, all the coefficients are summed up, and the resulting sum is referred to as "oddsSum". The product of all coefficients is also calculated, denoted as "oddsProd".
Next, to determine the bet amount for each sub-bet, the following formula is applied:
...
Then, to calculate the odds with margin (each sub-bet has its own margin corresponding to the ordinary condition), the following formula is used:
