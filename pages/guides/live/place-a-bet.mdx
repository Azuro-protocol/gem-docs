...
<Callout type="info">
  Azuro Live Betting API is currently deployed at:

  ```
  https://live-api.azuro.org/api/v1/public
  ```

  This url is subject to change when we go to production environment.
  In the code below we will use `API_BASE_URL` variable for this url.
</Callout>
