...
For decoding error code, please note that the error can be transmitted from different ABI (ProxyFront (if used), LP, Core) and may be stored in a sublevel of an error.

### Example of getting contract error

```ts copy filename=getContractErrorInfo.ts
import { decodeErrorResult, isHex } from 'viem'
import { coreABI, freebetABI, proxyFrontABI, lpv2ABI } from 'path/to/abi'

const abis = [ coreABI, lpv2ABI, freebetABI, proxyFrontABI ]
