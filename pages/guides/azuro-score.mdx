...
Use [fetch method above](#method-params-and-result-types) without params to get total leaderboard.
...
Use [fetch method above](#method-params-and-result-types) with `{ address }` param.
...
Use [fetch method above](#method-params-and-result-types) with `{ since, until }` params.
...
Use [fetch method above](#method-params-and-result-types) with `{ since, until, address }` params.
Current Azuro Score multiplier period is WEEK, so pass **MONDAY's 00:00 timestamp in seconds**.
