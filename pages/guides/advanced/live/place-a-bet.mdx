...
socket.onmessage = (message) => {
  ...
  data.forEach(data => {
    if (data.outcomes) { // new odds received
      ...
      oddsData.outcomes = data.outcomes.reduce((acc, { id, price, clearPrice, maxStake }) => {
        acc[id] = {
          odds: price,
...
const response = await fetch(`https://preprod-api.azuro.org/api/v1/public/orders`, {
  method: 'POST',
...
const response = await fetch(`https://preprod-api.azuro.org/api/v1/public/orders/${orderId}`);
