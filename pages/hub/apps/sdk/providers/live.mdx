...
<Callout type="info">
  The `isLive` state can only be `true` when you're using the `Gnosis Mainnet` or the `Polygon Mumbai Testnet`. If you change the `appChain` to a different chain, `isLive` will automatically become `false`. Also, if you set the state of `isLive` to `true`, it will automatically switch the appChain to applicable network.
</Callout>
