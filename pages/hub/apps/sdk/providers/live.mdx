...
<Callout type="info">
  The `isLive` state can only be `true` supported chains. If you change the `appChain` to a different chain, `isLive` will automatically become `false`. Also, if you set the state of `isLive` to `true`, it will automatically switch the appChain to applicable network.
</Callout>
<Callout type="info">
  Supported chains: 
  - `Gnosis Mainnet`
