...
conditions.forEach((condition) => {
  ...
  condition.outcomes.forEach((outcome) => {
    ...
    const marketName = customMarketName || getMarketName({ outcome.outcomeId })

    // If customSelectionName has a value, then use it as selectionName; otherwise, use the getSelectionName function.
    const selectionName = customSelectionName || getSelectionName({ outcome.outcomeId, withPoint: true })
  })
})
```
