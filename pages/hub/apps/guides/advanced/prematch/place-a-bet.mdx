...
<Callout type="info">
  Note that data structure differs from the structure of [placing single bet on LP contract](/guides/advanced/prematch/place-a-bet#place-a-single-bet).
</Callout>
...
[subscribing to odds updates](/guides/advanced/prematch/real-time-odds-updates), you can keep the odds values up-to-date in real-time,
which can help reduce the risk of user frustration or missed betting opportunities. Alternatively, you can
[fetch the odds values](/guides/advanced/prematch/get-odds-values) before a user submits a bet to ensure that the odds values are current
