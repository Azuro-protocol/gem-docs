...
Instead of charging a fee in native tokens when a user sends a transaction, the `Relayer` uses bet tokens as a fee.

We've Implemented API for retrieval `Relayer` fee amount:

```ts
// For pre-production
const apiUrl = 'https://dev-api.azuro.org/api/v1/public/orders'
// For production
...
That `spendingAmount` you should to use for spending approval.
