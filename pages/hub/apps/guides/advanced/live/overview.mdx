...
- However, real-time market updates should be accessed via [Websocket API](https://gem.azuro.org/guides/advanced/live/data-retrieval#get-realtime-markets-updates) to ensure you get updates immediately, the instant they happen.
...
<Callout type="info">
When a game goes live, its ID is stored in a different smart contract. To display these live games in your UI, you'll need to fetch their details from the [LiveDataFeed](https://gem.azuro.org/guides/advanced/live/data-retrieval#how-to-display-live-games-and-markets) subgraph. Games should be filtered by 'Created' status for live display.
...
#### EIP712 Signature: 
Follow the [implementation guide](https://gem.azuro.org/guides/advanced/live/signature-format) and add a text message to capture user consent in the **`attention`** field (within 160 chars).

#### Error / bet status handling: 
Implement clear communication regarding any [errors](https://gem.azuro.org/guides/advanced/live/errors) or changes in bet status.
...
5. **Rejected**: bet order has been rejected, display the error message (make sure to formulate error messages that are clear for the users - find a list of errors [here](https://gem.azuro.org/guides/advanced/live/errors))
</Callout>
