...
```tsx {3,29,30,34,36-38,44,46} filename="src/components/Providers.tsx" copy
'use client'
...
```tsx {4, 5, 11, 16-19, 24, 30}  filename="src/app/layout.tsx" copy
...
```ts {5, 26} filename="src/components/index.ts" copy
import type { Metadata } from 'next'
import { cookies } from 'next/headers'
import { Inter } from 'next/font/google'
import './globals.css'
