...
Each bet may contain more than one selection (combo bets), so we'll made universal view for single and combo bets wil redeeming logic, for that purpose SDK has `useRedeemBet` hook.

```tsx {2, 40, 73-77} copy filename="src/components/BetCard.tsx"
'use client'
