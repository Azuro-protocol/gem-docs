...
If you're not familiar with the basics of Azuro Waves, read about it [here](/knowledge-hub/airdrops/azuro-waves/overview).
...
const baseUrl = `https://api.onchainfeed.org/api/v1/public/waves/${waveId}`

// waveId
// e.g.
// https://api.onchainfeed.org/api/v1/public/waves/1
// https://api.onchainfeed.org/api/v1/public/waves/active
```
...
const fetchAzuroWavesUserStats = async (userAddress: `0x${string}`) => {
  const url = `https://api.onchainfeed.org/api/v1/public/waves/active/participants/${userAddress.toLowerCase()}/stats`
...
const activateAzuroWavesUser = async (userAddress: `0x${string}`) => {
  const url = `https://api.onchainfeed.org/api/v1/public/waves/active/participants/${userAddress.toLowerCase()}/activate`
  // GET method, not POST
  const response = await fetch(url)
...
const fetchAzuroWavesTotalLeaderboard = async (userAddress?: `0x${string}`) => {
  const url = userAddress
    ? `https://api.onchainfeed.org/api/v1/public/waves/active/leaderboard?address=${userAddress.toLowerCase()}`
    : 'https://api.onchainfeed.org/api/v1/public/waves/active/leaderboard'
...
const fetchAzuroWavesPeriods = async () => {
  const response = await fetch('https://api.onchainfeed.org/api/v1/public/waves/active/periods')
...
const fetchAzuroWavesPeriodLeaderboard = async (periodStartsAt: number, userAddress?: `0x${string}`) => {
  const url = userAddress
    ? `https://api.onchainfeed.org/api/v1/public/waves/active/periods/${periodStartsAt}/leaderboard?address=${userAddress.toLowerCase()}`
    : `https://api.onchainfeed.org/api/v1/public/waves/active/periods/${periodStartsAt}/leaderboard`
...
const fetchAzuroWavesLevels = async () => {
  const url = 'https://api.onchainfeed.org/api/v1/public/waves/active/levels'
  const response = await fetch(url)
