...
Please follow the [Azuro Waves Guidelines](/files/Azuro%20Waves%20Guidelines.pdf) in your UI.
...
<div className="mt-3 sm:max-w-96">
  <Card
    ...
    text="PDF"
    toTab="/files/Azuro%20Waves%20Guidelines.pdf"
  />
</div>
...
For working with Azuro Waves data, we'll utilize the public API. The base URL is:
...
We recommend using the `active` key for the `waveId` parameter to retrieve current wave data.
It's important to note that there may be periods between waves, during which the API will return `null`.
...
/**
  ...
  */
const fetchAzuroWavesPeriods = async (waveId: number | 'active' = 'active') => {
  const url = `https://api.azuro.org/api/v1/public/waves/${waveId}/periods`
...
At the end of a period, users receive additional bonus points (`points * positionMultiplier`), regardless of their level.
