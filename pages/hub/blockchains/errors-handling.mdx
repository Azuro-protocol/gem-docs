...
Occurs when attempting to withdraw payout of bet provided by Paymaster freebets mechanics, in case of freebet - the bet is locked at Paymaster.
...
### [0x827ab0f2] `OnlyPaymaster()`
This error is raised when not PayMaster address trying to call `betOrder`.
...
### [0x756eeec9] `BetAlreadyPaid()`
Raised when an attempt to cashout already paid bet.

<Callout type="advice">
    Check bet id.
</Callout>

### [0x000bce44] `BetAlreadyResolved()`
Raised when an attempt to cashout already resolved bet.

<Callout type="advice">
    Check bet id.
</Callout>

### [0xdf86ecd4] `BetNotOwned()`
Raised when an attempt to cashout not owned bet.

<Callout type="advice">
    Check bet id.
</Callout>

### [0xe9313359] `BetOwnerSignatureExpired()`
Raised when an attempt to cashout not owned bet.
...
### [0x6b372233] `BettingContractNotAllowed()`
Raised when an attempt to cashout not supported betting contract (AzuroBet).

<Callout type="advice">
    Check bet id.
</Callout>

### [0xcc696b5b] `ComboBetResolving()`
Raised when an attempt to cashout partially resolved combo bet - some of subbets already resolved.
...
Raised when an attempt to cashout with insufficient cashout balance.

<Callout type="advice">
    Fill up cashout contract balance.
</Callout>

### [0x90cccb0c] `InvalidBetOwnerSignature()`
Raised when an attempt to cashout with incorrect passed parameter `betOwnerSignature` as signature for the `order`.

<Callout type="advice">
    Check `order` and `betOwnerSignature` parameters correctness.
</Callout>

### [0x7a47c9a2] `InvalidChainId()`
Raised when an attempt to cashout with passed parameter `order.chainId` is not equal to the `block.chainId`.

<Callout type="advice">
    Check `order.chainId` parameter correctness.
</Callout>

### [0xfdfffca7] `InvalidOdds()`
Raised when an attempt to cashout with passed parameter `odds` is lower passed `item.minOdds`.

<Callout type="advice">
    Check `odds` parameter correctness.
</Callout>

### [0x7ab84302] `InvalidOddsCount()`
Raised when an attempt to cashout with passed parameters count of `odds` is not equal count of `item.minOdds`.
...
### [0x2f58c7cd] `InvalidOracleSignature()`
Raised when an attempt to cashout with incorrect Oracle address.

<Callout type="advice">
    Check `oracleSignature` parameter correctness.
</Callout>