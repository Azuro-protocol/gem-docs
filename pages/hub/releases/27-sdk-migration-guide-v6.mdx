...
The current SDK version is `v6.0.0-beta.1`, and the Toolkit version is `v5.0.0-beta.1`. Please note that some parts might have changed in next versions
</Callout>

## Key changes

### Cashout, Batch betting and freebets are temporarily unavailable

These features will be available during release

### Dependency Removal: `@apollo/client`

The heavyweight `@apollo/client` dependency has been removed, and all requests now go through [TanStack Query](https://tanstack.com/query/v5/docs/react).
