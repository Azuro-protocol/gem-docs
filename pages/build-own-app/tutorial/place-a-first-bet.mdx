                      ...
                      <div className="flex gap-1 w-full">
                        {
                          outcomes.map((outcome) => {
                            const { selectionName, odds } = outcome
...
<div className="mt-4 border border-gray-300 rounded-lg overflow-hidden">
  <Image className="w-full" src="/images/place-bet-modal.png" width={2536} height={1604} alt="" />
</div>
...
export default function usePlaceBet() {
  ...
  const allowance = rawAllowance && formatUnits(rawAllowance, USDC_DECIMALS)
